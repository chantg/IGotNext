...
try {
  ...
  if(htmlspecialchars($_GET["game"])=="" and htmlspecialchars($_GET["name"])!=""){
    $sql = "SELECT Title, Price, Console FROM Accessories WHERE Console = '".htmlspecialchars($_GET['name'])."'";
  }
  elseif(htmlspecialchars($_GET["game"])!="" and htmlspecialchars($_GET["name"])==""){
    $sql = "SELECT Title, Price, Console FROM Accessories WHERE Title LIKE '%".htmlspecialchars($_GET['game'])."%'";
  }
  else
  {
    $sql = "SELECT Title, Price, Console FROM Accessories";
  }
  ...
  if ($result->num_rows > 0) {
	...
	echo "<a href = '/accessory.php?Title=".$row['Title']."'>";
      echo "<div class = 'card'>";
      echo "<img src =". $resStr." class = 'cardPic' alt =". $row['Title'].">";
      echo "<h2>".$row['Title']."</h2> <p class='cPrice'>For " .$row['Console']."<br></br>Price: $".$row['Price']."</p>";
      //echo "<h1>".$row['Title']."</h1> <p class='price'>$".$row['Price']."</p><p>Some text about the game..</p>"
	echo "</a>";
printf("<button id='myBtn' onclick='addCart(\"%s\", \"%s\",\"%s\",\"%s\")'>Add to Cart<i class='fa fa-shopping-cart'></i></button>",$IP,$row['Title'], $resStr,$row['Price']);
