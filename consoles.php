...
try {
  ...
  if(htmlspecialchars($_GET["game"])=="" and htmlspecialchars($_GET["name"])!=""){
    $sql = "SELECT Title, Price, Console FROM Console WHERE Console = '".htmlspecialchars($_GET['name'])."'";
  }
  elseif(htmlspecialchars($_GET["game"])!="" and htmlspecialchars($_GET["name"])==""){
    $sql = "SELECT Title, Price, Console FROM Console WHERE Title LIKE '%".htmlspecialchars($_GET['game'])."%'";
  }
  else
  {
    $sql = "SELECT Title, Price, Console FROM Console";
  }
  ...
  if ($result->num_rows > 0) {
	...
	echo "<a href = '/console.php?Title=".$row['Title']."'>";
      echo "<div class = 'card'>";
