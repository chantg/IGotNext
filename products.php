...
foreach($items as $key => $value){
  $sqlCount = "SELECT COUNT(Title) AS NumberOfProducts FROM Products WHERE Title = '" . $key . "'";
  ...
  if(htmlspecialchars($_GET["game"])=="" and htmlspecialchars($_GET["name"])!="" and htmlspecialchars($_GET["type"])==""){
    $sql = "SELECT Title, Price, Console FROM Products WHERE Console = '".htmlspecialchars($_GET['name'])."'";
  }
  elseif(htmlspecialchars($_GET["game"])!="" and htmlspecialchars($_GET["name"])==""and htmlspecialchars($_GET["type"])==""){
    $sql = "SELECT Title, Price, Console FROM Products WHERE Title LIKE '%".htmlspecialchars($_GET['game'])."%'";
  }
  elseif(htmlspecialchars($_GET["game"])=="" and htmlspecialchars($_GET["name"])==""and htmlspecialchars($_GET["type"])!=""){
    $sql = "SELECT Title, Price, Console FROM Products WHERE Type LIKE '%".htmlspecialchars($_GET['type'])."%'";
  }
  else
  {
  ...
  if ($result->num_rows > 0) {
    ...
    { //ensure there's no duplicate entries are shown even if DB is filled with multiples
